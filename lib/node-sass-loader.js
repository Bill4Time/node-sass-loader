 ...
 */
var compileSass = function compileSass(options) {
	if(_.isNull(options) || _.isUndefined(options)) {
		throw ReferenceError('You MUST provide options to the "compileSass" function in order for it to work.');
	}
...
var compileDirectories = function compileDirectories(options) {
	...
	_.map(subdirectory_paths, function(subdirectory_path) {
		...
		var new_options = _.extend({}, options, { source_directory: subdirectory_path, destination_directory: new_destination_directory });
...
var compileDirectory = function compileDirectory(options) {
	...
	_.map(files, function(file) {
		...
		var rendered_css = nodeSass.renderSync({
			file: file,
			outputStyle: 'nested',
			stats: stats
		});
		...
		fs.writeFileSync(destination_filepath, rendered_css, { /* TODO: provide hook-in for options */ });
	});
};
...
node_sass_loader.compileSass = compileSass;
